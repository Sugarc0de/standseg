...
/* GDAL removed: gdal_io.c has a self-contained ENVI implementation. */
...
// gdal_io.c
uchar_t       **GDAL_read_image(const char *fname, int nlines, int nsamps,
                                int nbands, const char *interleave,
                                int header_offset);
void            GDAL_write_image(Seg_proc Spr, char *fname);
void            GDAL_process_headers(Seg_proc Spr);
