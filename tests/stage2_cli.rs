...
fn the_cli_reproduces_a_fixture_byte_for_byte() {
    ...
    ok(&[
        "--rmap", "tests/stage2/e2e_gsv/input/rmap",
        "--stage2", "tests/stage2/e2e_gsv/input/layer",
        "--n2", "50,8000",
        "-o", "gsv",
        "--outdir", d.to_str().unwrap(),
    ]);
...
fn one_invocation_equals_two() {
    ...
    let c = outdir("split");
    ok(&[
        "--rmap", a.join("a.rmap.51").to_str().unwrap(),
        "--stage2", SCENE,
        "--n2", "100,8000",
        "-o", "c",
        "--outdir", c.to_str().unwrap(),
    ]);
...
fn the_argument_rules_refuse_what_they_claim_to() {
    ...
    let cases: [(&str, Vec<&str>); 6] = [
        ("--stage2 given but no size rules",
         vec!["-t", "10", "-o", "x", "--outdir", od, "--stage2", "tests/stage2/e2e_gsv/input/layer",
              "tests/golden/misc/temp_byte_bip"]),
        ("--n2 given but no second-stage image",
         vec!["-t", "10", "-o", "x", "--outdir", od, "--n2", "50,8000",
              "tests/golden/misc/temp_byte_bip"]),
        ("--rmap skips stage 1, so there is nothing to do without --stage2",
         vec!["-o", "x", "--outdir", od, "--rmap", "tests/stage2/e2e_gsv/input/rmap"]),
        ("--rmap skips stage 1, so -t has nothing to apply to",
         vec!["-t", "10", "-o", "x", "--outdir", od, "--rmap", "tests/stage2/e2e_gsv/input/rmap",
              "--stage2", "tests/stage2/e2e_gsv/input/layer", "--n2", "50,8000"]),
        ("no input image",
         vec!["-t", "10", "-o", "x", "--outdir", od]),
        ("cannot be combined",
         vec!["-t", "10", "-o", "x", "--outdir", od, "-A", "--n2", "50,8000",
              "--stage2", "tests/golden/misc/temp_byte_bip", "tests/golden/misc/temp_byte_bip"]),
    ];
    for (want, args) in cases {
        let o = run(&args);
        assert!(!o.status.success(), "expected a refusal for {args:?}");
        let err = String::from_utf8_lossy(&o.stderr);
        assert!(err.contains(want), "expected {want:?} in stderr, got:\n{err}");
    }
}
...
fn a_mismatched_grid_is_refused() {
    let d = outdir("grid");
    let o = run(&[
        "-t", "10", "-o", "x", "--outdir", d.to_str().unwrap(),
        "--stage2", "tests/stage2/e2e_gsv/input/layer",  // 200x200
        "--n2", "50,8000",
        "tests/golden/misc/temp_byte_bip",               // 250x250
    ]);
    assert!(!o.status.success(), "a 200x200 second image was accepted for a 250x250 scene");
    let err = String::from_utf8_lossy(&o.stderr);
